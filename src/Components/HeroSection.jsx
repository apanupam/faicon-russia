...
const StatBadge = ({ number, label }) => {
  ...
  return (
    <div className="px-3 md:px-4 py-2">
      <div className="text-2xl sm:text-4xl font-bold tet-[#FCFCFC]">{count.toLocaleString()}+</div>
      <div className="text-sm sm:text-md text-[#D9D9D9] font-normal">{label}</div>
    </div>
  );
};
...
const HeroSlider = () => {
  ...
  return (
    <section className="bg-[#091F5B] py-7">
      <div className="relative container mx-auto overflow-hidden rounded-md border border-[#363636] p-8 sm:p-12">
        ...
        <div className="relative flex flex-col justify-center items-start text-white z-10">
          {/* Title */}
          <h1 className="text-lg sm:text-xl text-lg:2xl font-medium mb-4 text-white">Falcon Russia</h1>

          {/* Subtitle */}
          <h2 className="text-3xl sm:text-4xl md:text-5xl xl:text-6xl font-bold mb-2 text-white">
            A DEPENDABLE PARTNER
          </h2>
          <p className="text-sm sm:text-md xl:text-lg font-normal mb-4 text-[#CFD2D7]">FOR THE OIL AND GAS INDUSTRY</p>

          {/* Contact Button */}
          <Link to='/contactus' className="bg-white w-fit text-blue-900 px-4 lg:px-6 py-3 rounded-sm font-semibold hover:bg-gray-100 transition-colors mb-12 flex items-center">
            Contact Us <MdArrowOutward  className="ms-2"/>
          </Link>

          {/* Industries Section */}
          <div className="w-full grid grid-cols-1 xl:grid-cols-2 mt-30">
            <div className="mb-4">
              <Swiper
            ...
            {/* Stats Badges */}
            <div className="flex space-x-4 h-fit mt-5 xl:mt-0">
            <div className="bg-white/10 backdrop-blur-sm rounded-lg px-2 md:px-4 py-2 border border-[#81818180] grid grid-cols-3">
